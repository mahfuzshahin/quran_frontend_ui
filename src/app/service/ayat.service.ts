...
export class AyatService {
  ...
  getAyat() {
    return this.httpClient.get<any>('http://localhost:3000/api/ayat' ).pipe(
  ...
  getAyatBySurah(surahId:any) {
    return this.httpClient.get<any>(`${environment.api_url}/ayats/by-surah/${surahId}`).pipe(
      catchError((error: any): Observable<any> => {
        console.log(error)
        if (error.status === 404) {
          console.log(error.error.message || 'No employees found.');
