...
export class SurahService {
  ...
  getSurah() {
    return this.httpClient.get<any>(environment.api_url+'/surahs' ).pipe(
  ...
  getAyatBySurah(surah_id:any) {
    return this.httpClient.get<any>(environment.api_url+'/ayats/by-surah/surah_id='+surah_id ).pipe(
      catchError((error: any): Observable<any> => {
        console.log(error)
        if (error.status === 404) {
          console.log(error.error.message || 'No employees found.');
  ...
  postAyat(ayat: Ayat){
    ...
    return this.httpClient.post(environment.api_url +'/ayat', body,{headers}).pipe(catchError((error:any, caught:Observable<any>):Observable<any> =>{
      if(error.status === 406){
