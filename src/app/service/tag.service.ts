...
export class TagService {
  ...
  getTag() {
    return this.httpClient.get<any>(environment.api_url+'/tags' ).pipe(
  ...
  getAyatByTag(tag_id:any) {
    return this.httpClient.get<any>(environment.api_url+'/ayats/by-tag/tag_id='+tag_id ).pipe(
      catchError((error: any): Observable<any> => {
        console.log(error)
        if (error.status === 404) {
          console.log(error.error.message || 'No employees found.');
